...
public class MeshData {
    ...
    Vector3[] CalculateNormals() { //ep.12 
        ...
        for (int i = 0; i < triangleCount; i++){
            normTringIdx = i * 3;
            vertxIdxA = triangles[normTringIdx];
            vertxIdxB = triangles[normTringIdx + 1];
            vertxIdxC = triangles[normTringIdx + 2];
            triangleNormal = surfaceNormalFromIndices(vertxIdxA, vertxIdxB, vertxIdxC);
        ...
        for (int i = 0; i < borderTriangleCount; i++){
            normTringIdx = i * 3;
            vertxIdxA = borderTriangles[normTringIdx];
            vertxIdxB = borderTriangles[normTringIdx + 1];
            vertxIdxC = borderTriangles[normTringIdx + 2];
            triangleNormal = surfaceNormalFromIndices(vertxIdxA, vertxIdxB, vertxIdxC);
            if (vertxIdxA >= 0){
                vertexNormals[vertxIdxA] += triangleNormal;
                vertexNormals[vertxIdxA].Normalize();
