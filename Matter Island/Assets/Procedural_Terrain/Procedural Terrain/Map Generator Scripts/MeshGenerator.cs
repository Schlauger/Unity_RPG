...
public static class MeshGenerator
{
    public static MeshData GenerateTerrainMesh(float[,] heightMap,float heightGain, AnimationCurve _heightGainCurve, int levelOfDetail){
        ...
        for (int x = 0; x < borderSize; x += meshSimplificationIncrement)
        {
            ...
            height = heightGainCurve.Evaluate(heightMap[x, y]) * heightGain;
            vertexIdx = vertexIndicesMap[x, y];//ep.12
            //meshData.uvs[vertexIdx] = new Vector2( ((width-1)-x) / (float)width, y / (float)height); //meshData.addUV(vertexIdx,(x)/ (float)borderSize, y/(float)borderSize);
            //meshData.vertices[vertexIdx] = new Vector3(minX + x,heightMap[x,y],minZ-y); //meshData.addVertex(vertexIdx, (float)minX + (float)x, (float)heightGainCurve.Evaluate(heightMap[x, y]) * (float)heightGain, (float)minZ - (float)y);
